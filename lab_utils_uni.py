...
from lab_utils_common import compute_cost, dlblue, dlorange, dldarkred, dlmagenta, dlpurple, dlcolors
...
def mk_cost_lines(x,y,w,b, ax):
    ...
    for p in zip(x, y):
        ...
        ax.annotate(f'{c_p_txt:0.0f}', xy=cxy, xycoords='data', color=dlpurple, xytext=(5, 0), textcoords='offset points')
        cstr += f"{c_p_txt:0.0f} +"
...
def plt_stationary(x_train, y_train):
    ...
    ax[1].text(.5, .95, "Click to choose w,b", bbox=dict(facecolor='white', ec='black'), fontsize=10, transform=ax[1].transAxes, verticalalignment='center', horizontalalignment='center')
    ax[2].plot_surface(tmp_w, tmp_b, z,  cmap = dlcm, alpha=.3, antialiased=True)
    ax[2].plot_wireframe(tmp_w, tmp_b, z, color='k', alpha=.1)
...
class plt_update_onclick:
    ...
    def __call__(self, event):
        if event.inaxes == self.ax[1]:
            ...
            d = self.ax[1].annotate(f"Cost: {cst:.0f}", xy=(ws, bs), xytext=(4, 4), textcoords='offset points', bbox=dict(facecolor='white'), size=10)
...
def soup_bowl():
    ...
    W, B = np.meshgrid(w, b)
    ax.plot_surface(W, B, z, cmap = "Spectral_r", alpha=.7, antialiased=False)
    ax.plot_wireframe(W, B, z, color='k', alpha=.1)
...
def plt_contour_wgrad(x, y, hist, ax, w_range=[-100, 500, 5], b_range=[-500, 500, 5], contours=[.1, 50, 1_000, 5_000, 10_000, 25_000, 50_000],
    ...
    CS = ax.contour(w0, b0, z, contours, linewidths=2, colors=[dlblue, dlorange, dldarkred, dlmagenta, dlpurple])
...
def plt_divergence(p_hist, J_hist, x_train, y_train):
    ...
    fix_b = 100
    w_array = np.arange(-70_000, 70_000, 1_000)
    ...
    tmp_b, tmp_w = np.meshgrid(np.arange(-35_000, 35_000, 500), np.arange(-70_000, 70_000, 500))
    z = np.zeros_like(tmp_b)
    ...
    ax.plot_surface(tmp_w, tmp_b, z, alpha=.3, color=dlblue)
    ax.xaxis.set_major_locator(MaxNLocator(2))
...
def add_line(dj_dx, x1, y1, d, ax):
    ...
    ax.annotate(r"$\frac{\partial J}{\partial w}$ =%d" % dj_dx, fontsize=14, xy=(x1, y1), xycoords='data', xytext=(xoff, 10), textcoords='offset points', arrowprops=dict(arrowstyle="->"),
...
def plt_gradients(x_train,y_train, f_compute_cost, f_compute_gradient):
    ...
    ax[1].quiverkey(Q, .9, .9, 2, r'$2 \frac{m}{s}$', labelpos='E', coordinates='figure')
